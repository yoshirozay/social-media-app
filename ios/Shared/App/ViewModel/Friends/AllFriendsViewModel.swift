...
class AllFriendsObservable: ObservableObject {
    ...
    init() {
        anyCancellable = friendsDictionary.objectWillChange.sink { [weak self] (_) in
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.02) {
                ...
                for i in self.friendsDictionary.friendsDictionary.keys {
                    ...
                    if count-1 == self.friendsDictionary.friendsDictionary.keys.count && !people.isEmpty {
                        self.rows.append(people)
                        people.removeAll()
