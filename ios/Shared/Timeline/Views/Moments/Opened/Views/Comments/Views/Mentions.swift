...
struct MutualFriendsForMentions: View {
    ...
    var body: some View {
        let mutuals = mutualFriends.mutualFriends.values.filter{$0.username.lowercased().contains(self.content.components(separatedBy: "@")[content.indicesOf(string: "@").count].lowercased())}
        if !mutuals.isEmpty {
        ScrollView() {
            VStack {
